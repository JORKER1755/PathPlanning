...
"""shared_cnn与cnn的唯一差别就在于StateModel是否添加@shared"""
"""共享的实现除了需要装饰器@shared还需要保证第一个位置参数可以用来区分eval_model和target_model，保证模型不在这两者间共享"""

@shared
class StateModel:
    def __init__(self, eval_model: bool, size_splits, n_filters=5):
        """eval_model必须作为第一个参数：用来保证eval_model与target_model不共享本模型"""
